  ...
  useEffect(() => {
    ...
    return(<>
       ...
       <div className="flex flex-col mt-2 px-2">
         ...
         <p className="text-3xl text-gray-600 ">Hi, <span className="font-bold text-teal-400"> Jenifer!</span></p>
         ...
         <div className=" h-[300px] ">
          ...
          {/* SERACH INPUT! */}
           <div className="flex items-center justify-between mt-2">   
             <input className="border-2 border-teal-400 bg-gray-100 outline-none placeholder:text-gray-400 w-[68%] rounded-full p-1 px-4 " placeholder={"Search by year"} type="text"/>
             <button className="border-2 border-teal-400 w-[30%] bg-gradient-to-r from-teal-300 to-teal-400 p-1 rounded-full font-semibold text-white " >Search</button>
           </div>
        ...
        <button className="bg-green-500 text-white px-4 py-2 rounded-md">Get Started</button>
           ...
           {/* SHOW CROPS! */}
            <div className="h-36 mt-2 flex gap-1 w-full flex-wrap">
              ...
              {/* ADD NEW! */}
              <div onClick={() => setAuthState({open:true})} className="items-center flex-col gap-1 justify-center h-36 rounded-md bg-gradient-to-r  from-teal-300 to-green-300  w-[49%] overflow-hidden relative flex">
      
      <img src={'add-file.png'} alt="new-document" className="w-10 h-10" />
      <p className="text-center text-white font-semibold text-xl">Add New Crop Entry</p>
      </div>
             {/* BOX! */}
             

             {/* VIEW CROPS! */}
              { crops?.length>0 && crops?.map((c:any,i:any) => (<div key={i} 
              className="h-36 rounded-md bg-red-500 w-[49%] overflow-hidden relative flex">
              <img src={'wheat-1.jpg'} alt="'whaet" className=" object-fit w-full h-full" />
              <Link href={`/ali/${c?._id}`} className="items-center flex gap-4 absolute font-semibold text-xl bottom-0 right-0 left-0 bg-gradient-to-r from-[#F0FFF4] via-[#FFDFE5] to-[#E6E6FA] text-black  p-2 opacity-70 w-full">
                <p className="">{c?.name}</p>
                <p className="text-sm">({c?.month} - {c?.year})</p>
              </Link>
             </div>))
              }
