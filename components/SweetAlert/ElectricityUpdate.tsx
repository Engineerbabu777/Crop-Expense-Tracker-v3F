...
export default function Update({closeButton,prevData}:Props){
    ...
    const updateData = async() => {
        ...
        fetch('/api/elect',{
            ...
            body:JSON.stringify({bill:data.amount , units:data.units , date:data.date , id:prevData?._id , desc:data?.desc })
    ...
    <p className="text-start mb-1">Enter Units:</p>
    <input type="number" name="units" value={data?.units} onChange={handler}className="w-full p-2 outline-none border-2 border-green-500 rounded-md" placeholder="Enter Updated Units"/>
