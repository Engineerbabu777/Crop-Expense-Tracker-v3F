...
export default function Update({closeButton,prevData}:Props){
    ...
    const updateData = async() => {
        ...
        fetch('/api/dies',{
            ...
            body:JSON.stringify({amount:data.amount , liters:data.liters , date:data.date , id:prevData?._id , desc:data?.desc })
    ...
    <p className="text-start mb-1">Enter Liters:</p>
    <input type="number" name="liters" value={data?.liters} onChange={handler}className="w-full p-2 outline-none border-2 border-green-500 rounded-md" placeholder="Enter Updated Units"/>
